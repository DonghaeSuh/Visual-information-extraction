...
def read_ground_truths(test_data_dir: str):
    ...
    for filename in glob(test_data_dir):
        with open(filename, "r") as f:
            data = json.load(f)
        filename = os.path.splitext(os.path.basename(filename))[0] # only filename
        ...
        if len(line) == 3:
            ...
            if pred_label != "O":
                if pred_label == "S-COMPANY":
                    ...
                    pred_label = "total"
                pr_parses[filename][pred_label].append(text) # append text to the corresponding label
        elif len(line) == 2:
            raise NotImplementedError(f"{output_path} is op test dataset.")

    for (filename, pr_parse) in pr_parses.items():
        for (pred_label, value) in pr_parse.items(): # pr_parse : {'company': [~~], 'date': [~~], 'address': [~~], 'total': [~~]} 
...
def evaluation(gt_parses, pr_parses: str, **kwargs):
    '''
    gt_parses : dict : {'filename': {'company': '~~', 'date': '~~', 'address': '~~', 'total': '~~'}, ...}
    pr_parses : dict : {'filename': {'company': '~~', 'date': '~~', 'address': '~~', 'total': '~~'}, ...}
    '''
    ...
    assert len(entity_score_per_entity.keys()) == len(total_per_entity.keys()) # dict_keys(['company', 'date', 'address', 'total])
